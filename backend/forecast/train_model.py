...
def evaluate(name, model):
    ...
    return {"mae": round(float(mae), 2), "rmse": round(float(rmse), 2), "mape": round(float(mape), 1)}

metrics_linear = evaluate("Linear Regression", linear_model)
metrics_forest = evaluate("Random Forest", forest_model)

best_model = forest_model if metrics_forest["mae"] < metrics_linear["mae"] else linear_model
best_name = "random_forest" if best_model is forest_model else "linear_regression"
best_metrics = metrics_forest if best_model is forest_model else metrics_linear
